...
@param [in]     Z      Z座標
"""
function exact_solution!(e::Array{Float64,3}, ox, Δh, Z::Vector{Float64}, par::String)
...
function solveSOR!(θ, λ, b, mask, Δh, ω, Z, ΔZ,
    ...
    res0 = resSOR(θ, λ, b, mask, Δh, ω, Z, ΔZ, z_range, HF, HT, par)
    if res0==0.0
        res0 = 1.0
    end
    ...
    for n in 1:Constant.ItrMax
        #res = sor!(θ, λ, b, mask, Δh, ω, Z, ΔZ, z_range, HF, HT, par) / res0
        res = rbsor!(θ, λ, b, mask, Δh, ω, Z, ΔZ, z_range, HF, HT, par) / res0
...
function rbsor!(θ::Array{Float64,3},
    ...
    for c in 0:1
        res += rbsor_core!(θ, λ, b, mask, Δh, ω, Z, ΔZ, z_range, HF, HT, c, par)
    end
...
function PBiCGSTAB!(X::Array{Float64,3},
    ...
    for itr in 1:Constant.ItrMax
        rho = Fdot2(pcg_r, pcg_r0, z_range, par) # 非計算部分はゼロのこと
        ...
        Preconditioner!(pcg_p_, pcg_p, λ, mask, Δh, smoother, Z, ΔZ, z_range, HF, HT, par)

        CalcAX!(pcg_q, pcg_p_, Δh, λ, mask, Z, ΔZ, z_range, HT, par)
        alpha = rho / Fdot2(pcg_q, pcg_r0, z_range, par)
        r_alpha = -alpha
        Triad!(pcg_s, pcg_q, pcg_r, r_alpha, z_range, par)

        pcg_s_ .= 0.0  #fill!(pcg_s_, 0.0)
        Preconditioner!(pcg_s_, pcg_s, λ, mask, Δh, smoother, Z, ΔZ, z_range, HF, HT, par);
        ...
        Triad!(pcg_r, pcg_t_, pcg_s, r_omega, z_range, par)
...
function Preconditioner!(xx::Array{Float64,3},
    ...
    if smoother=="gs"
        for _ in 1:LCmax
            #sor!(xx, λ, bb, mask, Δh, 1.0, Z, ΔZ, z_range, HF, HT, par)
